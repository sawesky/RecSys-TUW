...
from ebrec.utils._articles_behaviors import map_list_article_id_to_value #povezi article_id iz behaviora.parquet sa articles.parquet i to kolone koje oces iz articles.parquet 
...
from ebrec.utils._constants import (
    ...
    DEFAULT_LABELS_COL, #ovo smo napravili 1/0 listu kad smo ucitavali
    DEFAULT_USER_COL,
...
class NewsrecDataLoader(tf.keras.utils.Sequence):
    ...
    sc_col: str = DEFAULT_SCROLL_PERCENTAGE_COL
...
@dataclass
class NRMSDataLoader(NewsrecDataLoader): #a sto radi dva puta .pipe to mi nije jasno 
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        return df.pipe(
            map_list_article_id_to_value,
            behaviors_column=self.history_column,
            mapping=self.lookup_article_index, #pokupi ono mapiranje za artikle
            fill_nulls=self.unknown_index, #sta je unknown rokni na "zeros", to smo tako definisali u nmrs_ebnerd.ipynb
            drop_nulls=False,
        ).pipe(
            map_list_article_id_to_value,
    ...
    def __getitem__(self, idx) -> tuple[tuple[np.ndarray], np.ndarray]: #hvataj iteme po batchevima
        """
...
class LSTURDataLoader(NewsrecDataLoader):
    ...
    user_id_mapping: dict[int, int] = field(default_factory=dict)
    unknown_user_value: int = 0

    def __post_init__(self):
        super().__post_init__()
        if not self.user_id_mapping:
            self.user_id_mapping = {user_id: user_id for user_id in self.behaviors[self.user_col].unique().to_list()}
    
    def map_user_ids(self, user_id):
        return self.user_id_mapping.get(user_id, self.unknown_user_value)
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        return (
            ...
            .with_columns(
                pl.col(self.user_col).apply(self.map_user_ids).alias(self.user_col)
            )
        )
...
@dataclass(kw_only=True)
class GRUSCDataLoader(NewsrecDataLoader):
    ...
    def transform(self, df: pl.DataFrame) -> pl.DataFrame:
        return (
            ...
            .with_columns(
                pl.col(self.user_col).apply(self.map_user_ids).alias(self.user_col),
                pl.col(self.sc_col).fill_null(0.0).alias(self.sc_col)
            )
        )
    ...
    def __getitem__(self, idx) -> tuple[tuple[np.ndarray], np.ndarray]:
        ...
        if self.eval_mode:
            ...
            scroll_percentages = np.log1p(np.array(
                batch_X.select(
                    pl.col(self.sc_col).repeat_by(pl.col("n_samples")).explode()
                )[self.sc_col].to_list()
            ).reshape(-1, 1))
        else:
            batch_y = np.array(batch_y.to_list())
            ...
            pred_input_title = np.squeeze(pred_input_title, axis=2)
            scroll_percentages = np.log1p(np.array(batch_X[self.sc_col].to_list()).reshape(-1, 1))

        his_input_title = np.squeeze(his_input_title, axis=2)

        return (user_indexes, his_input_title, pred_input_title, scroll_percentages), batch_y
